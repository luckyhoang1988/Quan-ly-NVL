...
- [x] **[Bổ sung 2026-07-26, không thuộc FR gốc] PR routed qua `Approval` + phạm vi xem + `Supplier.managed_by` + PO auto-numbering**: PR duyệt qua cơ chế `Approval` dùng chung với GRN/GIN — chỉ quản lý phòng Mua hàng hoặc Manager/Admin duyệt được, role PURCHASING thường không tự duyệt kể cả khi PR chỉ định đúng mình (`assigned_to` chỉ mang tính thông báo). Phạm vi xem PR giới hạn theo `requested_by`/`assigned_to`, trừ quản lý/Manager/Admin xem toàn bộ. Role PURCHASING được tạo `Supplier` (tự gán `managed_by`). `PurchaseOrder.po_no` tự sinh `PO-XXXX`, không nhập tay. Chi tiết đầy đủ + các bản vá liên quan: xem `CLAUDE.md` § Non-obvious cross-cutting design decisions.
...
- [x] **FR-PO-02** `MUST` — Khi tồn < Min Level, gợi ý tạo PO tự động — MVP không Celery: nút "Tạo yêu cầu mua hàng" ở dòng dưới Min Level trên `inventory_list.html` → tạo PR (`origin=MIN_LEVEL`) → PO thật chỉ tạo sau khi PR được duyệt qua `po_create?from_pr=<pk>` (không có lối tắt tạo PO thẳng). Chi tiết: xem `CLAUDE.md` § Non-obvious cross-cutting design decisions.
